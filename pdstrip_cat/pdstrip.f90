...
!
!Catamaran extension (2026): Two identical demihulls placed symmetrically at y=+/-hulld.
!  Input: set catamaran=.true. and hulld=distance from CL to demihull center on line 6 of pdstrip.inp.
!  Requires sym=.false. (full section offsets) and npres>0 (pressure calculation).
!  hulld must exceed the maximum half-beam of all sections to avoid hull overlap.
!  The BEM solver addedmassexcitationcat models both hulls with independent sources using pqb/pqnb.
!  Drift forces use explicit two-hull pressure integration with port hull data from mirror-angle symmetry.
!  Pressure output (pressuretransfer) contains starboard hull only; port hull pressures can be derived
!  by symmetry: pres_port(i,mu) = pres_stb(npres+1-i, -mu).
!
...
integer, parameter:: nofmax=400  !max. number of offset points on a section and the free surface
...
logical:: catamaran              !true if twin-hull (catamaran) configuration
...
if (catamaran) write(6,'(a)')' Port hull: pres_port(i,mu) = pres_stb(npres+1-i, -mu)'
if (catamaran.and.hulld<=0) call stop1('hulld must be positive for catamaran')
if (.not.catamaran) hulld=0
...
if (lsect) then
 write(20,'(a)')text                                      !write initial data on file sectionsresults
endif
...
if (catamaran.and.sym) call stop1('Catamaran requires sym=.false. (full section offsets)')
if (catamaran.and.npres==0) call stop1('Catamaran requires npres>0 for pressure calculation')
...
Sections: do ise=1,nse
 ...
 if (ngap>0) tcross(ise)=tcross(ise)-merge(1.,0.5,sym)* &
  ...
  if (catamaran) then                                         !catamaran: two identical demihulls
   area(ise)=2*area(ise)                                      !double area for two hulls
   ys(ise)=0                                                  !catamaran is symmetric about y=0
   zs(ise)=2*zs(ise)                                         !double the z-moment (same zs after /area)
  endif
...
enddo Sections
if (catamaran) then                                              !check that hulls do not overlap
 do ise=1,nse
  if (hulld<maxval(yof(ise,1:nof(ise)))) then
   write(6,'(a,i3,a,f8.3)')' *** Section',ise,' max half-beam',maxval(yof(ise,1:nof(ise)))
   call stop1('hulld too small: demihulls overlap. hulld must exceed max half-beam of all sections.')
  endif
 enddo
endif
if (lsect) &
...
integer:: jj,iw,jw                     !indices for symmetry enforcement
logical:: issym                        !true if section is geometrically symmetric about y=0
...
real:: symtol                          !tolerance for symmetry detection
...
!Detect if section is geometrically symmetric about y=0 (for sym=.false. full sections)
issym=.false.
if (.not.sym .and. .not.catamaran .and. npres>0 .and. mod(nof1,2)==0) then
 symtol=0.01*maxval(abs(yof1(1:nof1)))       !tolerance = 1% of max half-beam
 issym=.true.
 do i=1,nof1/2
  jj=nof1+1-i
  if (abs(yof1(i)+yof1(jj))>symtol .or. abs(zof1(i)-zof1(jj))>symtol) then
   issym=.false.; exit
  endif
 enddo
endif
write(6,'(a,l2,a,i3,a,i3,l2,l2,i4)') ' issym=',issym,' nof1=',nof1,' npres=',npres,sym,catamaran,mod(nof1,2)
Frequencies: do iom=1,nfre
  ...
  elseif (catamaran) then Symmetry                               !twin-hull BEM with interaction
   ...
   call addedmassexcitationa(nof1,yof1,zof1,om(iom),addedm(1,1,iom),diff(1,1,iom), &
    frkr(1,1,iom),pr(1,1,iom),ileft,aint)
   !--- Enforce pressure symmetry for geometrically symmetric sections ---
   !For a symmetric section solved with the asymmetric BEM (sym=.false.),
   !numerical asymmetry in the source distribution causes port/starboard
   !pressure differences that are amplified by logarithmic interpolation.
   !Radiation modes: heave (col 2) is symmetric, sway (col 1) and roll (col 3) antisymmetric.
   !Diffraction: wave at angle mu_j pairs with angle -mu_j at the mirror pressure point.
   EnforceSymmetry: if (issym .and. npres>0) then
    !Radiation pressure: heave (symmetric)
    do ii=1,npres/2
     jj=npres+1-ii
     prtmp(1)=(pr(ii,2,iom)+pr(jj,2,iom))/2
     pr(ii,2,iom)=prtmp(1)
     pr(jj,2,iom)=prtmp(1)
    enddo
    !Radiation pressure: sway (antisymmetric)
    do ii=1,npres/2
     jj=npres+1-ii
     prtmp(1)=(pr(ii,1,iom)-pr(jj,1,iom))/2
     pr(ii,1,iom)=prtmp(1)
     pr(jj,1,iom)=-prtmp(1)
    enddo
    !Radiation pressure: roll (antisymmetric)
    do ii=1,npres/2
     jj=npres+1-ii
     prtmp(1)=(pr(ii,3,iom)-pr(jj,3,iom))/2
     pr(ii,3,iom)=prtmp(1)
     pr(jj,3,iom)=-prtmp(1)
    enddo
    !Diffraction pressure: pair wave angles wangl(iw) with wangl(jw)=-wangl(iw)
    !pr(ii,3+iw) at angle mu should equal pr(npres+1-ii,3+jw) at angle -mu
    do iw=1,nmu
     !Find paired angle jw such that wangl(jw) = -wangl(iw)
     jw=0
     do jj=1,nmu
      if (abs(wangl(jj)+wangl(iw))<0.01) then; jw=jj; exit; endif
     enddo
     if (jw==0 .or. jw<iw) cycle     !skip if no pair found or already processed
     if (iw==jw) then                 !self-paired angle (wangl=0): symmetric diffraction
      do ii=1,npres/2
       jj=npres+1-ii
       prtmp(1)=(pr(ii,3+iw,iom)+pr(jj,3+iw,iom))/2
       pr(ii,3+iw,iom)=prtmp(1)
       pr(jj,3+iw,iom)=prtmp(1)
      enddo
     else                             !paired angles: cross-enforce symmetry
      do ii=1,npres
       jj=npres+1-ii
       prtmp(1)=(pr(ii,3+iw,iom)+pr(jj,3+jw,iom))/2
       pr(ii,3+iw,iom)=prtmp(1)
       pr(jj,3+jw,iom)=prtmp(1)
      enddo
     endif
    enddo
   endif EnforceSymmetry
 endif Symmetry
enddo Frequencies
...
!that occurred with pqs/pqsn mp=+1 when hulls are separated (mirror potential nearly constant).
...
integer:: nbody                        !number of body panel sources (both hulls, excl. gaps)
...
integer:: kp                           !port hull offset index
integer:: iofs                         !offset for outboard/tunnel FS source indexing
real:: aint(nprmax)                    !interpolation factor for pressure point between offsets
real:: yof1(nofmax),zof1(nofmax)       !y and z coordinate of offset points (demihull-local)
real:: ys(nofmax)                      !y coords of stb hull in vessel frame
real:: yp(nofmax)                      !y coords of port hull in vessel frame (mirrored, reversed)
real:: zp(nofmax)                      !z coords of port hull (reversed order)
real:: t                               !water depth
...
real:: h,hp                            !point distance on free surface near to body
...
real:: yq(0:nofmax),zq(0:nofmax)       !y and z coordinates of sources (vessel frame)
...
complex:: pr(nprmax,3+nmumax)          !pressure amplitudes at pressure points (starboard hull)
complex:: pr_port(nprmax,3+nmumax)     !pressure amplitudes at pressure points (port hull)
complex:: phiatoffsets(nofmax)         !potential amplitudes at offset points
...
!Build vessel-frame coordinates for both hulls
!Stb hull: ys(k) = yof1(k) - hulld.  Offsets go from inboard waterline (k=1) to outboard (k=nof1).
!Port hull: mirror of stb hull about y=0 with reversed traversal for consistent outward normals.
!  yp(j) = -ys(nof1+1-j) = hulld - yof1(nof1+1-j)
!  zp(j) = zof1(nof1+1-j)
!Port hull goes from outboard waterline (j=1) around keel to inboard waterline (j=nof1).
do k=1,nof1
 ys(k)=yof1(k)-hulld
 yp(k)=hulld-yof1(nof1+1-k)
 zp(k)=zof1(nof1+1-k)
enddo
...
TwoFreeSurfaceDiscretisations: do nf=25,28,3
 ...
 !--- Build grid points yg,zg and source points yq,zq ---
 !Body sources: stb hull panels 1..nof1-1, then port hull panels 1..nof1-1
 j=0
 BodySourcesStb: do k=1,nof1-1
  if (ngap>0.and.any(gap(1:ngap)==k)) cycle
  j=j+1
  yq(j)=(ys(k)+ys(k+1))/2-(zof1(k+1)-zof1(k))/20
  zq(j)=(zof1(k)+zof1(k+1))/2+(ys(k+1)-ys(k))/20
 enddo BodySourcesStb
 BodySourcesPort: do k=1,nof1-1
  if (ngap>0.and.any(gap(1:ngap)==(nof1-k))) cycle             !mirror gap indices for port hull
  j=j+1
  yq(j)=(yp(k)+yp(k+1))/2-(zp(k+1)-zp(k))/20
  zq(j)=(zp(k)+zp(k+1))/2+(yp(k+1)-yp(k))/20
 enddo BodySourcesPort
 nbody=j

 !Outboard free surface: two wings extending away from the outer waterlines.
 !Stb outboard: from ys(nof1) extending toward more negative y (outboard in vessel frame).
 !Port outboard: from yp(1) extending toward more positive y.
 h=sqrt((ys(nof1)-ys(nof1-1))**2+(zof1(nof1)-zof1(nof1-1))**2)       !near distance at stb outer wl
 hp=h                                                                  !same for port (symmetric hull)
 iofs=nbody                                                            !source index offset
 OutboardFS: do k=1,nfs
  h=min(h*1.5,hend)
  hp=min(hp*1.5,hend)
  !Stb outboard grid points (extending toward more negative y from ys(nof1))
  yg(2*nof1+k)=merge(ys(nof1),yg(2*nof1+k-1),k==1)-h
  zg(2*nof1+k)=zwl
  yq(iofs+k)=yg(2*nof1+k)+0.5*h                                      !source midway
  zq(iofs+k)=zwl-1.0*h
  !Port outboard grid points (extending toward more positive y from yp(1))
  yg(2*nof1+nfs+k)=merge(yp(1),yg(2*nof1+nfs+k-1),k==1)+hp
  zg(2*nof1+nfs+k)=zwl
  yq(iofs+nfs+k)=yg(2*nof1+nfs+k)-0.5*hp
  zq(iofs+nfs+k)=zwl-1.0*hp
 enddo OutboardFS
 ...
 do k=1,nof1
  yg(k)=ys(k); zg(k)=zof1(k)
 enddo
 do k=1,nof1
  yg(nof1+k)=yp(k); zg(nof1+k)=zp(k)
 enddo

 a(0:nsrc-1,nsrc:nsrc+2+nmu)=0.                                       !inhomogeneous terms = 0

 !--- Body boundary conditions and Froude-Krilow forces ---
 !Starboard hull body panels (k=1..nof1-1)
 j=0
 BodyBCStb: do k=1,nof1-1
  if (ngap>0.and.any(gap(1:ngap)==k)) cycle
  j=j+1
  dy=ys(k+1)-ys(k)
  dz=zof1(k+1)-zof1(k)
  a(j,nsrc  )=-ciom*dz                                                                    !sway
  a(j,nsrc+1)=+ciom*dy                                                                    !heave
  a(j,nsrc+2)=+ciom*(ys(k+1)**2-ys(k)**2+zof1(k+1)**2-zof1(k)**2)/2                      !roll
  WaveAnglesStb: do iw=1,nmu
   ...
   zw1a=-conjg(zw1)
   zw2=exp(waven*cmplx(-zof1(k)+zwl,ys(k)*sinw))* &
    merge((exp(zw1)-1.)/zw1,1.+0.5*zw1,abs(zw1)>0.01)
   zw2a=exp(waven*cmplx( zof1(k)-zwl,ys(k)*sinw))* &
    merge((exp(zw1a)-1.)/zw1a,1.+0.5*zw1a,abs(zw1a)>0.01)
   a(j,nsrc+2+iw)=-fact1*waven/om*cmplx(dy,sinw*dz)*zw2*(0.,1.)
   frkr1(1,iw)=frkr1(1,iw)+fact1*zw2*dz*rho
   frkr1(2,iw)=frkr1(2,iw)-fact1*zw2*dy*rho
   frkr1(3,iw)=frkr1(3,iw)-fact1*zw2*(ys(k+1)**2-ys(k)**2+zof1(k+1)**2-zof1(k)**2)/2*rho
   ShallowStb: if (fact2/=0) then
    a(j,nsrc+2+iw)=a(j,nsrc+2+iw)+fact2*waven/om*cmplx(dy,-sinw*dz)*zw2a*(0.,1.)
    frkr1(1,iw)=frkr1(1,iw)+fact2*zw2a*dz*rho
    frkr1(2,iw)=frkr1(2,iw)-fact2*zw2a*dy*rho
    frkr1(3,iw)=frkr1(3,iw)-fact2*zw2a*(ys(k+1)**2-ys(k)**2+zof1(k+1)**2-zof1(k)**2)/2*rho
   endif ShallowStb
  enddo WaveAnglesStb
  ColumnsStb: do i=0,nsrc-1
   a(j,i)=pqnb(ys(k),zof1(k),ys(k+1),zof1(k+1),yq(i),zq(i))
  enddo ColumnsStb
 enddo BodyBCStb

 !Port hull body panels (k=1..nof1-1, using yp/zp arrays)
 BodyBCPort: do k=1,nof1-1
  if (ngap>0.and.any(gap(1:ngap)==(nof1-k))) cycle                    !mirror gap indices
  j=j+1
  dy=yp(k+1)-yp(k)
  dz=zp(k+1)-zp(k)
  a(j,nsrc  )=-ciom*dz                                                                    !sway
  a(j,nsrc+1)=+ciom*dy                                                                    !heave
  a(j,nsrc+2)=+ciom*(yp(k+1)**2-yp(k)**2+zp(k+1)**2-zp(k)**2)/2                          !roll
  WaveAnglesPort: do iw=1,nmu
   sinw=sin(wangl(iw)*pi/180)
   zw1=cmplx(-waven*dz,waven*sinw*dy)
   if (abs(zw1)>51.) call stop1('Frequency too high')
   if (abs(zw1).lt.0.001) zw1=0.001
   zw1a=-conjg(zw1)
   zw2=exp(waven*cmplx(-zp(k)+zwl,yp(k)*sinw))* &
    merge((exp(zw1)-1.)/zw1,1.+0.5*zw1,abs(zw1)>0.01)
   zw2a=exp(waven*cmplx( zp(k)-zwl,yp(k)*sinw))* &
    merge((exp(zw1a)-1.)/zw1a,1.+0.5*zw1a,abs(zw1a)>0.01)
   a(j,nsrc+2+iw)=-fact1*waven/om*cmplx(dy,sinw*dz)*zw2*(0.,1.)
   frkr1(1,iw)=frkr1(1,iw)+fact1*zw2*dz*rho
   frkr1(2,iw)=frkr1(2,iw)-fact1*zw2*dy*rho
   frkr1(3,iw)=frkr1(3,iw)-fact1*zw2*(yp(k+1)**2-yp(k)**2+zp(k+1)**2-zp(k)**2)/2*rho
   ShallowPort: if (fact2/=0) then
    a(j,nsrc+2+iw)=a(j,nsrc+2+iw)+fact2*waven/om*cmplx(dy,-sinw*dz)*zw2a*(0.,1.)
    frkr1(1,iw)=frkr1(1,iw)+fact2*zw2a*dz*rho
    frkr1(2,iw)=frkr1(2,iw)-fact2*zw2a*dy*rho
    frkr1(3,iw)=frkr1(3,iw)-fact2*zw2a*(yp(k+1)**2-yp(k)**2+zp(k+1)**2-zp(k)**2)/2*rho
   endif ShallowPort
  enddo WaveAnglesPort
  ColumnsPort: do i=0,nsrc-1
   a(j,i)=pqnb(yp(k),zp(k),yp(k+1),zp(k+1),yq(i),zq(i))
  enddo ColumnsPort
  enddo BodyBCPort

 !--- Free surface conditions --- !--- Free surface conditions ---
 !Outboard stb: panels from yg(2*nof1+1) to yg(2*nof1+nfs), extending toward more negative y
 !Near FS condition (first nf panels)
 OutboardStbNear: do k=1,min(nf,nfs)
  k1=2*nof1+k
  dy=yg(k1)-merge(ys(nof1),yg(k1-1),k==1)                            !dy < 0 (toward more negative y)
  do i=0,nsrc-1
   a(nbody+k,i)=pqnb(merge(ys(nof1),yg(k1-1),k==1),zwl,yg(k1),zwl,yq(i),zq(i)) &
    +om**2/g*abs(dy)*(pqb(merge(ys(nof1),yg(k1-1),k==1)+0.316*dy,zwl,yq(i),zq(i)) &
    +pqb(merge(ys(nof1),yg(k1-1),k==1)+0.684*dy,zwl,yq(i),zq(i)))/2
  enddo
 enddo OutboardStbNear
 !Far FS condition (radiation condition, panels nf+1 to nfs)
 OutboardStbFar: do k=nf+1,nfs
  k1=2*nof1+k
  dy=yg(k1)-yg(k1-1)                                                  !dy < 0
  factor=(real(k-nf)/(nfs-nf))**2
  do i=0,nsrc-1
   a(nbody+k,i)=waven*abs(dy)* &
  ...
  enddo
 enddo OutboardStbFar

 !Outboard port: panels from yg(2*nof1+nfs+1) to yg(2*nof1+2*nfs), extending toward more positive y
 OutboardPortNear: do k=1,min(nf,nfs)
  k1=2*nof1+nfs+k
  dy=yg(k1)-merge(yp(1),yg(k1-1),k==1)                               !dy > 0
  do i=0,nsrc-1
   a(nbody+nfs+k,i)=-pqnb(merge(yp(1),yg(k1-1),k==1),zwl,yg(k1),zwl,yq(i),zq(i)) &
    +om**2/g*abs(dy)*(pqb(merge(yp(1),yg(k1-1),k==1)+0.316*dy,zwl,yq(i),zq(i)) &
    +pqb(merge(yp(1),yg(k1-1),k==1)+0.684*dy,zwl,yq(i),zq(i)))/2
  enddo
 enddo OutboardPortNear
 OutboardPortFar: do k=nf+1,nfs
  k1=2*nof1+nfs+k
  dy=yg(k1)-yg(k1-1)                                                  !dy > 0
  factor=(real(k-nf)/(nfs-nf))**2
  do i=0,nsrc-1
   a(nbody+nfs+k,i)=waven*abs(dy)* &
    (pqb(yg(k1-1)+0.316*dy,zwl,yq(i),zq(i))+pqb(yg(k1-1)+0.684*dy,zwl,yq(i),zq(i)))/2 &
    -(0.,1.)*(pqb(yg(k1)-factor*dy,zwl,yq(i),zq(i))-pqb(yg(k1-1)-factor*dy,zwl,yq(i),zq(i)))
  enddo
 ...
 Forces: do m=1,3                                      !m=1 sway, m=2 heave, m=3 roll
  MotionsAndWaves: do l=1,3+nmu
   f=0
   !Starboard hull panels
   StbForce: do k=1,nof1-1
    if (ngap>0.and.any(gap(1:ngap)==k)) cycle
    dy=ys(k+1)-ys(k)
    dz=zof1(k+1)-zof1(k)
    if (m==1) then; fact=-dz
    elseif (m==2) then; fact=+dy
    else; fact=+0.5*(ys(k+1)**2-ys(k)**2+zof1(k+1)**2-zof1(k)**2)
    endif
    do i=0,nsrc-1
     f=f+a(i,nsrc-1+l)*fact*(pqb(ys(k)+0.316*dy,zof1(k)+0.316*dz,yq(i),zq(i)) &
      +pqb(ys(k)+0.684*dy,zof1(k)+0.684*dz,yq(i),zq(i)))/2
    enddo
   enddo StbForce
   !Port hull panels
   PortForce: do k=1,nof1-1
    if (ngap>0.and.any(gap(1:ngap)==(nof1-k))) cycle
    dy=yp(k+1)-yp(k)
    dz=zp(k+1)-zp(k)
    if (m==1) then; fact=-dz
    elseif (m==2) then; fact=+dy
    else; fact=+0.5*(yp(k+1)**2-yp(k)**2+zp(k+1)**2-zp(k)**2)
    endif
    do i=0,nsrc-1
     f=f+a(i,nsrc-1+l)*fact*(pqb(yp(k)+0.316*dy,zp(k)+0.316*dz,yq(i),zq(i)) &
      +pqb(yp(k)+0.684*dy,zp(k)+0.684*dz,yq(i),zq(i)))/2
    enddo
   enddo PortForce
   !Store results: 0.5 factor for nf averaging (no x2 symmetry factor — both hulls explicitly integrated)
   if (l<=3) then
    addedm1(m,l)=addedm1(m,l)+0.5*ciom*f/(-om**2)*rho
   else
 ...
 WithPressure: if (npres>0) then
  PressMotions: do l=1,3+nmu
   PressOffsets: do k=1,nof1
    phiatoffsets(k)=0.
    phiatoffsets_port(k)=0.
    PressSources: do i=0,nsrc-1
     phiatoffsets(k)=phiatoffsets(k)+a(i,nsrc-1+l)*pqb(ys(k),zof1(k),yq(i),zq(i))
     phiatoffsets_port(k)=phiatoffsets_port(k)+a(i,nsrc-1+l)*pqb(yp(k),zp(k),yq(i),zq(i))
    enddo PressSources
   enddo PressOffsets
   PressurePoints: do ii=1,npres
    pr(ii,l)=pr(ii,l)-ciom*rho*(phiatoffsets(ileft(ii)-1)+ &
     aint(ii)*(phiatoffsets(ileft(ii))-phiatoffsets(ileft(ii)-1)))/2
    pr_port(ii,l)=pr_port(ii,l)-ciom*rho*(phiatoffsets_port(ileft(ii)-1)+ &
     aint(ii)*(phiatoffsets_port(ileft(ii))-phiatoffsets_port(ileft(ii)-1)))/2
   enddo PressurePoints
  enddo PressMotions
 endif WithPressure
...
! Integration over omega for all responses
 ...
 Frequencies: do iom=1,nom
  ...
  !Note: sway drift interpolation through roll resonance is done in Phase 1 (post-Wavelengths).
  !barfeta already uses feta_stored (module array) which contains the interpolated values.
 ...
 Speed: do iv=1,nv
  ...
  IfPressures: if (npres>0) then
   ...
   enddo Pressures
   write(6,'(a,f12.3)')' Longitudinal drift force         ',2*barfxi(iv)
   write(6,'(a,f12.3)')' Transverse drift force           ',2*barfeta(iv)
   write(6,'(a,f12.3)')' Longitudinal water drift velocity',2*bardotxdr
   write(6,'(a,f12.3)')' Transverse water drift velocity  ',2*bardotydr
  endif IfPressures
 enddo Speed
...
complex:: vbody(3)              !complex body velocity amplitude at triangle centroid
complex:: vdotn                 !complex normal component of body velocity (v_body dot n)
...
real:: feta_before_tri          !feta before triangle loop (for debug)
real:: feta_WL, fxi_WL          !waterline-only contributions to drift forces
real:: fxi_WL_nopst, feta_WL_nopst  !WL drift without pst (diagnostic)
real:: dfxistb_np, dfxiprt_np, dfeta_np  !no-pst WL drift increments
real:: fxi_vel, fxi_rot         !velocity and rotation contributions to hull drift (fxi only)
      real:: feta_vel, feta_rot        !velocity and rotation contributions to sway drift
      real:: feta_rot_withpst          !rotation term WITH pst (diagnostic comparison)
real:: fxi_vel_x, fxi_vel_yz   !velocity x-component and yz-component decomposition
real:: fxi_vel_tang, fxi_vel_norm  !tangential-only vs normal-only velocity contributions
real:: df_velx_only  !temp for x-gradient-only velocity
real:: fxi_rot_nopst, feta_rot_nopst   !rotation term without pst (surge and sway)
real:: feta_rot_norollpst              !rotation term without roll-pst only
real:: df_rot_nopst(3)          !temp for no-pst rotation cross product
real:: df_rot_withpst(3)        !temp for with-pst rotation cross product
real:: df_rot_norollpst(3)      !temp for no-roll-pst rotation cross product
complex:: presavg_nopst         !pressure average without pst
complex:: presavg_norollpst     !pressure average without roll-pst
real:: fxi_boese_wp             !Boese waterplane integral contribution
real:: fxi_boese                !Boese total = waterplane + Gerritsma-Beukelman
complex:: cpres_fy_sum, cpres_fz_sum  !1st-order pressure force (y,z) summed over triangles
complex:: cnewton_fy, cnewton_fz      !Newton's 2nd law prediction: -ome^2*M*eta
complex:: cpres_fy_wg, cpres_fz_wg   !wave (excitation) contribution to panel force
complex:: cpres_fy_rad, cpres_fz_rad  !radiation contribution to panel force
complex:: cpres_fy_pst, cpres_fz_pst  !hydrostatic restoring contribution to panel force
complex:: ceom_fy_rad, ceom_fy_rst    !EOM-based radiation and restoring forces (sway)
complex:: ceom_fz_rad, ceom_fz_rst    !EOM-based radiation and restoring forces (heave)
complex:: ceom_fy_exc, ceom_fz_exc    !EOM-based excitation forces
complex:: presavg_wg, presavg_rad, presavg_pst  !triangle-averaged pressure components
complex:: cfrestore(3)             !balanced restoring force vector (buoyancy - weight) from szw
real:: df_rot_restore(3)           !rotation drift correction from balanced restoring (Option C fix)
real:: feta_rot_fixC               !sway rotation term with Option C fix (for diagnostic comparison)
real:: fxi_rot_fixC                !surge rotation term with Option C fix (for diagnostic comparison)
complex:: cnewton_force(3)         !Newton's 2nd law force: -ome^2 * M * eta (translational DOFs)
real:: df_rot_newton(3)            !rotation drift from Newton's 2nd law (Option 3)
real:: feta_rot_newton             !sway rotation from Newton (for diagnostic/actual use)
real:: fxi_rot_newton              !surge rotation from Newton (for diagnostic/actual use)
complex:: czeta_rel             !complex relative wave elevation at waterline
real:: dx2                      !distance between offset sections
real:: dfxistb,dfxiprt,dfeta    !Anteile Driftkraft
real:: tmpx
!--- Variables for resonance interpolation post-processing ---
real:: res_bw, res_om_lo, res_om_hi  !resonance window bounds
integer:: res_ilo, res_ihi           !frequency indices bounding resonance window
integer:: res_ileft, res_iright      !indices of good points outside window
real:: res_wt                        !interpolation weight
real:: res_feta_left, res_feta_right !feta at boundary points
...
complex,allocatable:: pres_nopst(:,:,:) !pressure without pst (for drift test)
complex,allocatable:: pres_norollpst(:,:,:) !pressure without roll-pst (pst_4 only excluded)
complex,allocatable:: pot(:,:)  !complex amplitudes of potential at pressure points
complex:: gradpot(3)            !tangential gradient of complex potential
complex:: gradpott(3)            !tangential gradient of complex potential
complex:: gradpotn(3)            !normal gradient of complex potential
...
logical:: it1
!--- Catamaran drift force variables ---
integer:: imumirror(nmumax)       !index of mirror angle (-mu) for each extended angle
integer:: imirr                   !mirror angle index for current angle
...
real:: zint_port(nprmax,nsemax)   !port hull z-coordinates of pressure points
complex:: pres_port_i             !port hull pressure at a point (from mirror angle)
complex:: pot_port_i              !port hull potential at a point (from mirror angle)
real:: pres_scr(2),pwg_scr(2),pot_scr(2) !scratch file read buffers (real,imag)
integer:: iscr1,iscr2,iscr3,iscr4 !scratch file header integers
complex:: calphaw_all(nfinmax,nmumax)     !stored calphaw per angle for catamaran deferred drift
complex:: cffw_all(nfinmax,nmumax)        !stored cffw per angle for catamaran deferred drift
complex:: cffs_all(1,6,nfinmax,nmumax)    !stored cffs per angle for catamaran deferred drift
...
complex:: vy_rel                   !relative sway velocity at a section
real:: wz_amp                      !wave vertical velocity amplitude (decay factor)
...
!--- Short-wave asymptotic added resistance variables ---
real:: fxi_sw_fmls                 !FMLS short-wave asymptotic surge drift
real:: feta_sw_fmls                !FMLS short-wave asymptotic sway drift
real:: fxi_sw_refl                 !Full reflection short-wave asymptotic surge drift
real:: feta_sw_refl                !Full reflection short-wave asymptotic sway drift
real:: dydx_stb, dydx_prt         !waterline slope dy/dx at starboard and port
real:: alpha_stb, alpha_prt       !waterline angle at starboard and port
real:: nwl_x, nwl_y               !outward waterline normal components
real:: ndotdw                      !n_wl dot d_wave (illumination check)
real:: sin2a                       !sin^2(alpha) for FMLS
real:: cos2fac                     !cos^2(theta)/|n|^2 factor for wall-normal force
real:: nmag2                       !|n_outward|^2 for waterline normal
...
open(33, file='normals.out', status='replace')
open(34, file="debug.out", status='replace')
it1=.true.
npres1=max(npres,1)
...
allocate(pres(npres1,1,nsemax),pres_nopst(npres1,1,nsemax),pres_norollpst(npres1,1,nsemax),pot(npres1,nsemax),cpfk(npres1))
...
OffsetSections: do ise1=nse,1,-1                                  !integral terms of restoring matrix
 ...
 yleft=yof(ise1,nof(ise1))
  ...
  if (catamaran) then                         !two identical demihulls at y=+/-hulld
   b3=2*b3+2*hulld**2*b1                     !parallel axis theorem: I_cat = 2*(I_demi + A_demi*d^2)
   b2=2*b2                                   !first moments cancel by symmetry; 2*b2_demi remains
   b1=2*b1                                   !double waterplane area
  endif
...
Sections: do ise1=1,nse                               !read and interpolate hydrodynamic section data
 ...
 if (npres>0 .and. catamaran) yint(1:npres,ise1)=yint(1:npres,ise1)-hulld  !shift to global coords (stb hull)
...
WaveAngles: do imu=nmu-i1,1+i2,-1
 ...
 mu(nmu)=pi-mu(imu)
enddo WaveAngles
!Build mirror angle index: imumirror(imu) = index of angle -mu(imu) (mod 2*pi)
if (catamaran) then
 imumirror=0
 do imu=1,nmu; do jmu=1,nmu
  if (abs(mu(imu)+mu(jmu))<0.001 .or. abs(mu(imu)+mu(jmu)-2*pi)<0.001) then
   imumirror(imu)=jmu; exit
  endif
 enddo; enddo
 do imu=1,nmu
  if (imumirror(imu)==0) call stop1('No mirror angle found for catamaran drift forces')
 enddo
 !Port hull pressure point coordinates: mirror of stb hull about y=0, reversed traversal
 do ise1=1,nse; do i=1,npres
  jj=npres+1-i
  yint_port(i,ise1)=-yint(jj,ise1)           !reflection about y=0
  zint_port(i,ise1)= zint(jj,ise1)           !same z
 enddo; enddo
endif
...
Wavelengths: do iom=1,nom                                                       !for all wavelengths
 ...
 omega_stored(iom) = om; nom_stored = max(nom_stored, iom)   !store frequency for resonance interp
 waveheight=min(maxheight,steepness*wavelength(iom))
  Speeds: do iv=1,nv                                                                  !for all speeds
   vs=speed(iv)
   if (catamaran.and.npres>0) open(30,status='scratch',form='unformatted')
   ...
   Iteration: do itz=1,200                                                !for <=200 iteration steps
    ...
    OffsetSects: do ise1=nse,1,-1                          !for all offset sections, beginning at bow
     FirstIteration: if (itz==1) then
      ...
      ncrossold(:,:,ise1)=ncross(:,:,ise1)
       ...
       if (catamaran) then                      !two hulls: double forces, add roll damping via hulld lever
        ncross(3,3,ise1)=2*hulld**2*zwmatr22(1,1) !roll-roll: 2*d^2*Dy (parallel axis, sway drag only)
        ncross(1,1,ise1)=2*ncross(1,1,ise1)     !double sway damping for two hulls
        ncross(2,2,ise1)=2*ncross(2,2,ise1)     !double heave damping for two hulls
       endif
       ...
       ecross(:,:,ise1)=fillcomplmatr(3,1,        (/ &
        ...
        0.5*rho*vcross*wdz*bcross(ise1)*cdz(ise1),    &
                     (0.,0.)                  /))
       if (catamaran) then                      !double cross-flow excitation for two hulls
        ecross(1,1,ise1)=2*ecross(1,1,ise1)     !sway (roll excitation zero by symmetry)
        ecross(2,1,ise1)=2*ecross(2,1,ise1)     !heave
       endif
      !BNV/2006-05-08: original code replaced by code to facilitate Salford FTN95 compiler
    ...
    if (itz.eq.1 .and. roll_damp_frac > 0.0 .and. abs(ome) > 1e-6) then
     ...
     if (c44 > 0 .and. a44 > 0) then
      b44crit = 2.0 * sqrt(a44 * c44)                   !critical roll damping
      b44ext = roll_damp_frac * b44crit                  !external damping = fraction * critical
      addedmass(4,4,1) = addedmass(4,4,1) - cmplx(0.0, ome*b44ext)  !add to imaginary part
     endif
     endif
     !--- Compute natural roll frequency (once) for resonance interpolation ---
     if (itz.eq.1 .and. omega_roll < 1e-6 .and. abs(ome) > 1e-6) then
      c44 = real(szw(4,4))
      a44 = real(massmatr(4,4,1)) + real(addedmass(4,4,1))/ome**2
      if (c44 > 0 .and. a44 > 0) then
       omega_roll = sqrt(c44 / a44)
   ...
   write(6,'(/'' Wave circ. frequency'',1f6.3,''  encounter frequ.'', &
    &f6.3,''  wave length'',f9.2,''  wave number'',f7.4,''  wave angle'',f6.1/ &
   ...
   IfPressure: if (npres.gt.0) then                               !Pressures using corrected motions
     Sectns: do ise1=1,nse
      ...
      pres(:,:,ise1)=pwg(:,:,ise1)+((prg(:,:,ise1)+pst(:,:,ise1)).mprod.motion)
       pres_nopst(:,:,ise1)=pwg(:,:,ise1)+(prg(:,:,ise1).mprod.motion)  !without pst for drift test
       !pres_norollpst: now exclude heave pst (pst_3 = ρg·η₃) — the actual blowup source
       pres_norollpst(:,:,ise1)=pres(:,:,ise1)-pst(:,3:3,ise1)*motion(3,1)  !subtract heave pst contribution
      pot(:,ise1)=reshape(((prw(:,:,ise1).mprod.motion)+pw(:,:,ise1))/rho,(/npres/))
       if (catamaran) then                                          !catamaran: defer writing to unit 24
        write(30)iom,iv,imu,ise1                                  !scratch file; copied to pressuretransfer
        do i=1,npres                                               !after WaveDir loop (CatDriftForces block)
         write(30)pres(i,1,ise1),pwg(i,1,ise1),pot(i,ise1)        !NOTE: starboard hull pressures only;
        enddo                                                      !port hull: pres_port(i,mu)=pres_stb(npres+1-i,-mu)
      else
     ...
     if (catamaran) then                                            !store for deferred drift force calc.
      ...
      !Determine drift forces (only if npres>0) --- monohull path, unchanged
          ...
          fxi_vel_x=0; fxi_vel_yz=0  !velocity x vs yz decomposition
           fxi_vel_tang=0; fxi_vel_norm=0  !tangential vs normal velocity decomposition
            cpres_fy_sum=0; cpres_fz_sum=0  !1st-order pressure force accumulator
            cpres_fy_wg=0; cpres_fz_wg=0; cpres_fy_rad=0; cpres_fz_rad=0
            cpres_fy_pst=0; cpres_fz_pst=0  !decomposed 1st-order forces
        write(34,'(a,f8.3,a,f8.1)') 'DRIFT_START omega=',om,' mu=',mu(imu)*180/3.14159265
     ForAllSections: do ise1=1,nse
      ...
      elseif (ise1==nse) then
       ...
       dyprt=(yint(npres,ise1+1)-yint(npres,ise1-1))/2
           ...
           dfxistb= 0.25*abs(pres(    1,1,ise1))**2*dystb/rho/g    !WL drift: use full pres (incl. hydrostatic restoring pst)
            ...
            !--- No-pst WL test ---
            dfxistb_np= 0.25*abs(pres_nopst(    1,1,ise1))**2*dystb/rho/g
            dfxiprt_np=-0.25*abs(pres_nopst(npres,1,ise1))**2*dyprt/rho/g
            fxi_WL_nopst=fxi_WL_nopst+dfxistb_np+dfxiprt_np
            dfeta_np=0.25*dx2*(-abs(pres_nopst(1,1,ise1))**2+abs(pres_nopst(npres,1,ise1))**2)/rho/g
            feta_WL_nopst=feta_WL_nopst+dfeta_np
            !--- end no-pst WL test ---
            !--- Boese waterplane integral ---
            ! Relative wave elevation at waterline (z=0, no depth decay):
            ! zeta_r(x) = wave(x) - ship_displacement(x)
            ! |zeta_r|^2 is the same regardless of sign convention
            czeta_rel = motion(3,1) - x(ise1)*motion(5,1) - czeta3(ise1)
            fxi_boese_wp = fxi_boese_wp - rho*g*0.25*abs(czeta_rel)**2*(dystb - dyprt)
            if (abs(om-0.446)<0.01 .and. abs(mu(imu)*180/3.14159265-180.0)<1.0 .and. abs(vs-3.0)<0.5) &
              write(34,'(a,i3,a,f8.2,a,g12.4,a,g12.4,a,g12.4)') &
          ...
          write(34,'(a,i3,a,f8.3,a,f8.4,a,f8.4,a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6)') &
           '  WL sec=',ise1,' dx2=',dx2,' dystb=',dystb,' dyprt=',dyprt, &
      ...
      PressureTriangles: do i=2,npres; do j=1,2                    !for 2 triangles per hull quadrilateral
       ...
       if (it1 .eqv. .true.) then
          write(33, *) ise1, i, j, (xn(1:3))
       endif
        gradvec=graddreieck(xtri)
        vbody=ciome*(motion(1:3,1)+(motion(4:6,1).vprod.cmplx(xc,(/0.,0.,0./))))  !complex body velocity at centroid
        vdotn=sum(vbody*xn)                                             !normal component: v_body dot n_outward
        gradpott=(pot(ip1,is1)-pot(ip3,is3))*gradvec(:,1)+(pot(ip2,is2)-pot(ip1,is1))*gradvec(:,2) ! tangential only
        gradpot=gradpott+vdotn*xn                                      !tangential + normal comp.
         presaverage=(pres(ip1,1,is1)+pres(ip2,1,is2)+pres(ip3,1,is3))/3
         presavg_nopst=(pres_nopst(ip1,1,is1)+pres_nopst(ip2,1,is2)+pres_nopst(ip3,1,is3))/3
         presavg_norollpst=(pres_norollpst(ip1,1,is1)+pres_norollpst(ip2,1,is2)+pres_norollpst(ip3,1,is3))/3

           df=-0.25*rho*sum(abs(gradpot)**2)*flvec  &  !velocity squared: full 3D (matches original)
                   -0.5*real((presaverage*flvec).vprod.conjg(motion(4:6,1)))  !rotation term: matches original (no negation)
          !--- Also compute x-gradient-only velocity term for comparison ---
          df_velx_only = -0.25*rho*abs(gradpot(1))**2*flvec(1)

        if (it1 .and. ise1==2) then
         write(34,'(a,2i3,a,3g14.6,a,3g14.6,a,g14.6,a,3g14.6,a,g14.6,a,g14.6)') &
          '  TRI',i,j,' fl=',flvec,' xn=',xn,' |gp|2=',sum(abs(gradpot)**2), &
          ' gp=',abs(gradpot),' df2=',df(2),' |vdotn|=',abs(vdotn)
        endif
        !--- Separate velocity term into x-component and yz-component for diagnosis ---
        fxi_vel_x = fxi_vel_x + (-0.25*rho*abs(gradpot(1))**2*flvec(1))
        fxi_vel_yz = fxi_vel_yz + (-0.25*rho*(abs(gradpot(2))**2+abs(gradpot(3))**2)*flvec(1))
        !--- Also track tangential-only velocity contribution ---
        fxi_vel_tang = fxi_vel_tang + (-0.25*rho*sum(abs(gradpott)**2)*flvec(1))
        fxi_vel_norm = fxi_vel_norm + (-0.25*rho*abs(vdotn)**2*flvec(1))

        fxi=fxi+df(1)
        feta=feta+df(2)
           fxi_vel=fxi_vel+(-0.25*rho*sum(abs(gradpot)**2)*flvec(1))
           fxi_rot=fxi_rot+(df(1)-(-0.25*rho*sum(abs(gradpot)**2)*flvec(1)))
           feta_vel=feta_vel+(-0.25*rho*sum(abs(gradpot)**2)*flvec(2))
           feta_rot=feta_rot+(df(2)-(-0.25*rho*sum(abs(gradpot)**2)*flvec(2)))
           df_rot_withpst=-0.5*real((presaverage*flvec).vprod.conjg(motion(4:6,1)))
          feta_rot_withpst=feta_rot_withpst+df_rot_withpst(2)
          df_rot_nopst=-0.5*real((presavg_nopst*flvec).vprod.conjg((motion(4:6,1))))
         fxi_rot_nopst=fxi_rot_nopst+df_rot_nopst(1)
         feta_rot_nopst=feta_rot_nopst+df_rot_nopst(2)
           df_rot_norollpst=-0.5*real((presavg_norollpst*flvec).vprod.conjg((motion(4:6,1))))
          feta_rot_norollpst=feta_rot_norollpst+df_rot_norollpst(2)
           !--- Accumulate 1st-order pressure force: F = +∮ p n_in dS (flvec points INWARD) ---
           cpres_fy_sum = cpres_fy_sum + presaverage*flvec(2)
           cpres_fz_sum = cpres_fz_sum + presaverage*flvec(3)
           !--- Decompose into wave, radiation, pst components ---
           presavg_wg = (pwg(ip1,1,is1)+pwg(ip2,1,is2)+pwg(ip3,1,is3))/3
           presavg_rad = (sum(prg(ip1,:,is1)*motion(:,1))+sum(prg(ip2,:,is2)*motion(:,1)) &
                         +sum(prg(ip3,:,is3)*motion(:,1)))/3
           presavg_pst = (sum(pst(ip1,:,is1)*motion(:,1))+sum(pst(ip2,:,is2)*motion(:,1)) &
                         +sum(pst(ip3,:,is3)*motion(:,1)))/3
           cpres_fy_wg  = cpres_fy_wg  + presavg_wg*flvec(2)
           cpres_fz_wg  = cpres_fz_wg  + presavg_wg*flvec(3)
           cpres_fy_rad = cpres_fy_rad + presavg_rad*flvec(2)
           cpres_fz_rad = cpres_fz_rad + presavg_rad*flvec(3)
           cpres_fy_pst = cpres_fy_pst + presavg_pst*flvec(2)
           cpres_fz_pst = cpres_fz_pst + presavg_pst*flvec(3)
       mdrift(1)=mdrift(1)+xc(2)*df(3)-xc(3)*df(2)
       mdrift(3)=mdrift(3)+xc(1)*df(2)-xc(2)*df(1)                                              !bezogen auf O
       ...
       enddo; enddo PressureTriangles
        write(34,'(a,i3,a,g14.6)') '  TRI_SUM sec=',ise1,' feta_tri=',feta-feta_before_tri
       enddo ForAllSections    
       !--- Newton rotation (diagnostic only, not applied to drift forces) ---
       ! Tested as Option 3: gives nearly identical results to panel rotation.
       ! The roll resonance blow-up is intrinsic to Pinkster near-field, not a bug.
       cnewton_force(1) = -ome**2*sum(massmatr(1,1:6,1)*motion(1:6,1))
       cnewton_force(2) = -ome**2*sum(massmatr(2,1:6,1)*motion(1:6,1))
       cnewton_force(3) = -ome**2*sum(massmatr(3,1:6,1)*motion(1:6,1))
         df_rot_newton = -0.5*real(cnewton_force.vprod.conjg(motion(4:6,1)))
       fxi_rot_newton = df_rot_newton(1)
       feta_rot_newton = df_rot_newton(2)
       !--- Option C diagnostic (not applied): ---
       ! The panel pst gives buoyancy-only. szw gives balanced (buoyancy-weight).
       ! But the rotation term needs the DISTRIBUTED pressure, not just the resultant force.
       ! So we can't simply replace panel pst with global szw - the spatial distribution matters.
       ! Compute correction for diagnostic comparison only:
       cfrestore(1) = sum(szw(1,:)*motion(:,1))
       cfrestore(2) = sum(szw(2,:)*motion(:,1))
       cfrestore(3) = sum(szw(3,:)*motion(:,1))
         df_rot_restore = -0.5*real(cfrestore.vprod.conjg(motion(4:6,1)))
       fxi_rot_fixC = fxi_rot_nopst + df_rot_restore(1)
       feta_rot_fixC = feta_rot_nopst + df_rot_restore(2)
       write(34,'(a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6)') &
          '  FIXC_RESTORE df_x=',df_rot_restore(1),' df_y=',df_rot_restore(2), &
          ' df_z=',df_rot_restore(3),' fxi_rot_fixC=',fxi_rot_fixC,' feta_rot_fixC=',feta_rot_fixC
       write(34,'(a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6)') &
          '  DRIFT_TOTAL fxi=',fxi,' feta=',feta,' fxi_WL=',fxi_WL,' feta_WL=',feta_WL, &
          ' fxi_vel=',fxi_vel,' fxi_rot=',fxi_rot
       write(34,'(a,g14.6,a,g14.6,a,g14.6,a,g14.6,a,g14.6)') &
          '  DRIFT_SWAY feta_vel=',feta_vel,' feta_rot=',feta_rot, &
          ' feta_rot_wpst=',feta_rot_withpst,' feta_rot_nopst=',feta_rot_nopst, &
          ' feta_rot_norpst=',feta_rot_norollpst
           write(34,'(a,g14.6,a,g14.6,a,g14.6)') &
            '  DRIFT_NOPST fxi_WL_nopst=',fxi_WL_nopst,' feta_WL_nopst=',feta_WL_nopst, &
            ' fxi_rot_nopst=',fxi_rot_nopst
           write(34,'(a,g14.6,a,g14.6)') &
            '  DRIFT_VEL_XYZ fxi_vel_x=',fxi_vel_x,' fxi_vel_yz=',fxi_vel_yz
            write(34,'(a,g14.6,a,g14.6)') &
             '  DRIFT_VEL_TN fxi_vel_tang=',fxi_vel_tang,' fxi_vel_norm=',fxi_vel_norm
         !--- Pressure-force consistency check: does sum(p*n*dS) = -ome^2*M*eta (Newton's 2nd law)? ---
         ! cnewton = -ome^2 * M_row * motion (using full 6x6 mass matrix for y and z rows)
         cnewton_fy = -ome**2*sum(massmatr(2,:,1)*motion(:,1))
         cnewton_fz = -ome**2*sum(massmatr(3,:,1)*motion(:,1))
         ! EOM-based forces: F_rad = (-ome^2*A + i*ome*B)*eta = -addedmass*eta  (addedmass = ome^2*A - i*ome*B)
         ceom_fy_rad = -sum(addedmass(2,:,1)*motion(:,1))
         ceom_fz_rad = -sum(addedmass(3,:,1)*motion(:,1))
         ! EOM-based restoring: F_rst = C*eta = szw*eta
         ceom_fy_rst = sum(szw(2,:)*motion(:,1))
         ceom_fz_rst = sum(szw(3,:)*motion(:,1))
         ! EOM-based excitation: F_exc = ex(:,1,1) for total ship (section index 1)
         ceom_fy_exc = ex(2,1,1)
         ceom_fz_exc = ex(3,1,1)
             write(34,'(a,2g14.6,a,2g14.6,a,2g14.6,a,2g14.6)') &
              '  FORCE_CHECK Fy_pres=',real(cpres_fy_sum),aimag(cpres_fy_sum), &
              ' Fy_newton=',real(cnewton_fy),aimag(cnewton_fy), &
              ' Fz_pres=',real(cpres_fz_sum),aimag(cpres_fz_sum), &
              ' Fz_newton=',real(cnewton_fz),aimag(cnewton_fz)
             write(34,'(a,2g14.6,a,2g14.6,a,2g14.6)') &
              '  FCHECK_WG Fy_wg=',real(cpres_fy_wg),aimag(cpres_fy_wg), &
              ' Fy_rad=',real(cpres_fy_rad),aimag(cpres_fy_rad), &
              ' Fy_pst=',real(cpres_fy_pst),aimag(cpres_fy_pst)
             write(34,'(a,2g14.6,a,2g14.6,a,2g14.6)') &
              '  FCHECK_WGZ Fz_wg=',real(cpres_fz_wg),aimag(cpres_fz_wg), &
              ' Fz_rad=',real(cpres_fz_rad),aimag(cpres_fz_rad), &
              ' Fz_pst=',real(cpres_fz_pst),aimag(cpres_fz_pst)
             write(34,'(a,2g14.6,a,2g14.6,a,2g14.6)') &
              '  FCHECK_EOM Fy_exc=',real(ceom_fy_exc),aimag(ceom_fy_exc), &
              ' Fy_rad=',real(ceom_fy_rad),aimag(ceom_fy_rad), &
              ' Fy_rst=',real(ceom_fy_rst),aimag(ceom_fy_rst)
             write(34,'(a,2g14.6,a,2g14.6,a,2g14.6)') &
              '  FCHECK_EOMZ Fz_exc=',real(ceom_fz_exc),aimag(ceom_fz_exc), &
              ' Fz_rad=',real(ceom_fz_rad),aimag(ceom_fz_rad), &
              ' Fz_rst=',real(ceom_fz_rst),aimag(ceom_fz_rst)
      !--- Maruo/Gerritsma-Beukelman far-field drift force ---
      ! R_aw = (ome^2 * k) / (2*om) * Sum b33(x) * |s_rel(x)|^2 * dx
      ! where s_rel(x) = eta3 - x*eta5 - zeta(x)*exp(-k*T(x))
      ! and b33 = section 2D heave damping
      ! Sign: R_aw is added resistance (positive opposing motion)
      ! In pdstrip convention: fxi_maruo = -R_aw (forward force is positive)
      fxi_maruo=0; fxi_maruo_heave=0; fxi_maruo_sway=0; feta_maruo=0
      MaruoSections: do ise1=1,nse
       dx2=(x(min(ise1+1,nse))-x(max(ise1-1,1)))/2
       ! Section heave damping b33: amatr_sec(2,2) = (A33 - i*B33/ome)/ome^2
       ! => B33 = -aimag(amatr_sec(2,2)) * ome^3
       b33_sec = -aimag(amatr_sec(2,2,ise1)) * abs(ome)**3
       ! Section sway damping b22:
       b22_sec = -aimag(amatr_sec(1,1,ise1)) * abs(ome)**3
       ! Section draft (max z below waterline, internal coords: z positive down)
       draft_sec = tcross(ise1)
       ! Wave decay factor at section draft (deep water)
       if (waven*(zbot-zwl)>3.0) then
        wz_amp = exp(-waven*draft_sec)   !decay of wave orbital to mean draft
       else
        wz_amp = cosh(waven*(draft_sec+zwl-zbot))/sinh(waven*(zbot-zwl))
       endif
       ! Relative heave displacement at section x:
       ! s_rel_z(x) = eta3 - x*eta5 - zeta(x)*exp(-k*T)
       ! motion(3,1)=heave RAO, motion(5,1)=pitch RAO (rad), czeta3=exp(-ik*x*cosm)
       vz_rel = motion(3,1) - x(ise1)*motion(5,1) - czeta3(ise1)*wz_amp
       ! Heave contribution to added resistance:
       ! dR = (ome^2 * k)/(2*om) * b33 * |s_rel|^2 * dx
       fxi_maruo_heave = fxi_maruo_heave + ome**2*waven/(2.*om)*b33_sec*abs(vz_rel)**2*dx2
       ! Sway relative displacement (for oblique seas):
       ! s_rel_y(x) = eta2 - x*eta4(~0) - zeta(x)*(-i*sinm)*exp(-k*T)
       ! Using sinm for the y-component of wave orbital
      ...
      write(34,'(a,g14.6,a,g14.6,a,g14.6)') &
        '  DRIFT_MARUO fxi_maruo=',fxi_maruo, &
        ' fxi_maruo_heave=',fxi_maruo_heave,' fxi_maruo_sway=',fxi_maruo_sway
       write(6,'(a,g14.6,a,g14.6)') &
         ' Maruo/G-B far-field surge drift force: ',fxi_maruo,' (heave+sway contrib: ', &
         fxi_maruo_heave+fxi_maruo_sway
       !--- end Maruo ---
       !--- Boese (1970) total: waterplane integral + Gerritsma-Beukelman ---
       ! fxi_boese_wp was accumulated in ForAllSections loop above
       ! Sign: fxi_boese_wp is positive forward (like all fxi terms)
       ! fxi_maruo is already negative for added resistance
       fxi_boese = fxi_boese_wp + fxi_maruo
       write(34,'(a,g14.6,a,g14.6,a,g14.6)') &
         '  DRIFT_BOESE fxi_boese=',fxi_boese, &
         ' fxi_boese_wp=',fxi_boese_wp,' fxi_maruo=',fxi_maruo
        !--- end Boese ---
        !--- Short-wave asymptotic added resistance (FMLS & full reflection) ---
       ! Integrates over the "illuminated" part of the waterline only.
       ! Wave propagation direction: d_wave = (cos(mu), sin(mu))
       !   mu=180 head seas: d_wave = (-1, 0) = waves traveling toward stern
       !
       ! For a 2D vertical wall, the mean drift force per unit length of wall
       ! per unit wave amplitude squared is:
       !   f = (1/2) * rho * g * cos^2(theta_n)
       ! where theta_n is the angle between the wave direction and the wall normal.
       ! The force direction is along the inward normal (pushing the wall).
       !
       ! FMLS (Faltinsen, Minsaas, Liapis & Skjordal, 1980): infinite draft limit.
       ! Full reflection: includes draft correction [1 - exp(-2*k*T*cos^2(theta_n))]
       !
       ! Convention: fxi positive = forward; resistance is negative fxi
       fxi_sw_fmls=0; feta_sw_fmls=0
       fxi_sw_refl=0; feta_sw_refl=0
        SWAsymSections: do ise1=1,nse
         sw_dx=(x(min(ise1+1,nse))-x(max(ise1-1,1)))/2
         bwl_stb = yint(1,ise1)           !starboard WL y-coordinate (positive)
         bwl_prt = yint(npres,ise1)       !port WL y-coordinate (negative)
         draft_sec = tcross(ise1)
         ! Waterline slopes dy/dx (central differences, forward/backward at ends)
         if (ise1==1) then
          dydx_stb = (yint(1,2)-yint(1,1))/(x(2)-x(1))
          dydx_prt = (yint(npres,2)-yint(npres,1))/(x(2)-x(1))
         elseif (ise1==nse) then
          dydx_stb = (yint(1,nse)-yint(1,nse-1))/(x(nse)-x(nse-1))
          dydx_prt = (yint(npres,nse)-yint(npres,nse-1))/(x(nse)-x(nse-1))
         else
          dydx_stb = (yint(1,ise1+1)-yint(1,ise1-1))/(x(ise1+1)-x(ise1-1))
          dydx_prt = (yint(npres,ise1+1)-yint(npres,ise1-1))/(x(ise1+1)-x(ise1-1))
         endif
         ! Diagnostic output for head seas, first speed only (cosm=-1 => mu=180)
         if (iv==1 .and. iom==1 .and. abs(cosm-(-1.0))< 0.001) then
          write(34,'(a,i3,a,f10.3,a,f10.4,a,f10.4,a,f8.3,a,f8.4)') &
           '  SW sec',ise1,' x=',x(ise1),' ystb=',bwl_stb,' yprt=',bwl_prt, &
           ' dydx_s=',dydx_stb,' dx=',sw_dx
         endif
         !
          ! --- Starboard side ---
          ! Outward normal: tangent = (1, dydx_stb), outward normal = (-dydx_stb, 1)
          nwl_x = -dydx_stb
          nwl_y = 1.0
          ndotdw = nwl_x*cosm + nwl_y*sinm
           if (ndotdw < 0.0) then
            ! Reflection force acts along the INWARD wall normal (pressure acts
            ! normal to surface).  Inward normal = (dydx_stb, -1) (unnormalized).
            ! dF = (1/2)*rho*g * cos^2(theta_i) * n_hat_inward * dl
            ! cos^2(theta_i) = ndotdw^2 / |n|^2,  dl = |n|*dx
            ! => dF = (1/2)*rho*g * ndotdw^2 / |n|^2 * n_inward * dx
            nmag2 = nwl_x**2 + nwl_y**2    ! |n_outward|^2
            cos_an = abs(ndotdw) / sqrt(nmag2)
            cos2fac = ndotdw**2 / nmag2     ! cos^2(theta) * (dl/dx / |n|) = ndotdw^2/|n|^2
            ! Inward normal for starboard: (dydx_stb, -1)
            fxi_sw_fmls = fxi_sw_fmls + 0.5*rho*g * cos2fac * dydx_stb * sw_dx
            feta_sw_fmls = feta_sw_fmls + 0.5*rho*g * cos2fac * (-1.0) * sw_dx
            ! Full reflection with draft correction:
            refl_factor = 1.0 - exp(-2.0*waven*draft_sec*cos_an**2)
            fxi_sw_refl = fxi_sw_refl + 0.5*rho*g * cos2fac * dydx_stb * sw_dx * refl_factor
            feta_sw_refl = feta_sw_refl + 0.5*rho*g * cos2fac * (-1.0) * sw_dx * refl_factor
            if (iv==1 .and. iom==1 .and. abs(cosm-(-1.0))< 0.001) then
             write(34,'(a,i3,a,f10.2,a,f8.4,a,f8.4,a,f10.2)') &
              '    STB illum sec',ise1,' dfxi=', &
              0.5*rho*g*cos2fac*dydx_stb*sw_dx, &
              ' cos_an=',cos_an,' ndotdw=',ndotdw,' cumfxi=',fxi_sw_fmls
            endif
           endif
          !
          ! --- Port side ---
         ! Outward normal: tangent = (1, dydx_prt), outward normal = (dydx_prt, -1)
         nwl_x = dydx_prt
         nwl_y = -1.0
         ndotdw = nwl_x*cosm + nwl_y*sinm
           if (ndotdw < 0.0) then
            nmag2 = nwl_x**2 + nwl_y**2
            cos_an = abs(ndotdw) / sqrt(nmag2)
            cos2fac = ndotdw**2 / nmag2
            ! Inward normal for port: (-dydx_prt, 1)
            fxi_sw_fmls = fxi_sw_fmls + 0.5*rho*g * cos2fac * (-dydx_prt) * sw_dx
            feta_sw_fmls = feta_sw_fmls + 0.5*rho*g * cos2fac * (1.0) * sw_dx
            refl_factor = 1.0 - exp(-2.0*waven*draft_sec*cos_an**2)
            fxi_sw_refl = fxi_sw_refl + 0.5*rho*g * cos2fac * (-dydx_prt) * sw_dx * refl_factor
            feta_sw_refl = feta_sw_refl + 0.5*rho*g * cos2fac * (1.0) * sw_dx * refl_factor
            if (iv==1 .and. iom==1 .and. abs(cosm-(-1.0))< 0.001) then
             write(34,'(a,i3,a,f10.2,a,f8.4,a,f8.4,a,f12.2)') &
              '    PRT illum sec',ise1,' dfxi=', &
              0.5*rho*g*cos2fac*(-dydx_prt)*sw_dx, &
              ' cos_an=',cos_an,' ndotdw=',ndotdw,' cumfxi=',fxi_sw_fmls
            endif
           endif
       enddo SWAsymSections
       write(34,'(a,g14.6,a,g14.6)') &
         '  DRIFT_SW_FMLS fxi=',fxi_sw_fmls,' feta=',feta_sw_fmls
       write(34,'(a,g14.6,a,g14.6)') &
         '  DRIFT_SW_REFL fxi=',fxi_sw_refl,' feta=',feta_sw_refl
       write(6,'(a,g14.6,a,g14.6)') &
         ' Short-wave FMLS asymptotic drift: fxi=',fxi_sw_fmls,' feta=',feta_sw_fmls
       write(6,'(a,g14.6,a,g14.6)') &
         ' Short-wave reflection asymptotic: fxi=',fxi_sw_refl,' feta=',feta_sw_refl
       !--- end short-wave asymptotic ---
       it1 = .false.
     findriftf=0
     FinDriftForce: do ifin=1,nfin
      if (abs(vs) .gt. 1.0) then
      ...
      fxi=fxi+findriftf(1); feta=feta+findriftf(2)
       !--- Blend Pinkster with short-wave REFL asymptote ---
       ! Save unblended Pinkster for diagnostic output
       fxi_pink = fxi; feta_pink = feta
        ! RAO-based smooth blending, restricted to near-head seas (|cosm|>0.85,
        ! mu~148-212 deg) where the bow entrance is the primary illuminated surface.
        ! At broader headings the hull side inflates REFL forces unrealistically.
         ! Blending ratio r = min(|z_bow|, 1) where z_bow = heave - pitch*x_bow per
         ! unit wave amplitude. REFL is ADDED to Pinkster (not averaged) because
         ! Pinkster does not capture reflection physics (gives negative at short waves).
         ! F_blend = F_Pinkster + (1-r)*F_REFL.
         ! At short waves r~0: F = Pinkster + REFL; at long waves r~1: F = Pinkster.
         ! Only applied to surge (fxi). Sway (feta) is left as pure Pinkster because
         ! the REFL sway correction is not standard in strip theory and the force
         ! direction (wall-normal) already ensures minimal surge from parallel hull.
         if (abs(cosm) > 0.85) then
           z_bow_complex = motion(3,1) - x(nse)*motion(5,1)
           bow_ratio = min(abs(z_bow_complex), 1.0)
           fxi  = fxi  + (1.0 - bow_ratio)*fxi_sw_refl
         endif
       !--- end blending ---
      write(6,'(a,2g14.6)')' Pinkster (unblended) drift force per wave amplitude squared        ', &
       fxi_pink,feta_pink
      write(6,'(a,2g14.6)')' Longitudinal and transverse drift force per wave amplitude squared ', &
       fxi,feta
     write(6,'(a,g14.6)')' Yaw drift moment per wave amplitude squared                      ',mdrift(3)
   ...
   !--- Catamaran deferred drift force computation ---
   !After WaveDir loop: for catamaran, copy scratch file to unit 24 with drift forces interleaved
   CatDriftForces: if (catamaran.and.npres>0) then
    rewind(30)
    ...
    CatAngles: do imu=1,nmu
     ...
     ciome=ci*ome
     imirr=imumirror(imu)
     !Copy pressure records from scratch file to unit 24
     do ise1=1,nse
      read(30) iscr1,iscr2,iscr3,iscr4
      write(24,*) iscr1,iscr2,iscr3,iscr4
      do i=1,npres
       read(30) pres_port_i, presaverage, pot_port_i    !reusing variables as scratch buffers
     ...
     CatStbSections: do ise1=1,nse
      dx2=(x(min(ise1+1,nse))-x(max(ise1-1,1)))/2
      if (ise1==1) then
       dystb=merge((yint(1,2)+yint(1,1))/2,(yint(1,2)-yint(1,1))/2,ltrwet(iv))
       dyprt=merge((yint(npres,2)+yint(npres,1))/2,(yint(npres,2)-yint(npres,1))/2,ltrwet(iv))
      elseif (ise1==nse) then
       dystb=((yint(1,nse)+yint(npres,nse))/2-(yint(1,nse-1)+yint(1,nse)))/2
       dyprt=((yint(1,nse)+yint(npres,nse))/2-(yint(npres,nse-1)+yint(npres,nse)))/2
      else
       dystb=(yint(1,ise1+1)-yint(1,ise1-1))/2
       dyprt=(yint(npres,ise1+1)-yint(npres,ise1-1))/2
     ...
     enddo CatStbSections
      ...
      CatFinDrift: do ifin=1,nfin
       if (abs(vs) .gt. 1.0) then
          calphas1=-ciome*sum(wfinmatr(1,:,ifin)*motion(:,1))*rfin(ifin)/(vs+1e-5) 
          calphas2=sum(rfin(ifin)*cfinmatr(:,:,ifin).mprod.motion)
          calphas3=sum(-skorfin(ifin)*delfin_all(:,:,ifin,imu).mprod.motion)
          cffast=conjg(cffw_all(ifin,imu)+sum(cffs_all(:,:,ifin,imu).mprod.motion))
          findriftf(1)=findriftf(1)+0.5*real((calphaw_all(ifin,imu)+calphas1)*cffast)
          findriftf=findriftf+0.5*rfin(ifin)*(real(wfinmatr(1,1:3,ifin)).vprod.real(motion(4:6,1)*cffast))
          findriftf(1)=findriftf(1)-bfin(ifin)*vs**2*clgrfin(ifin)/2/pi/lambdaeff(ifin) &
               *abs(calphaw_all(ifin,imu)+calphas1+calphas2+calphas3)**2
       endif
      enddo CatFinDrift
     fxi=fxi+findriftf(1); feta=feta+findriftf(2)
     write(6,'(a,2g14.6)')' Longitudinal and transverse drift force per wave amplitude squared ', &
 ...
 ResonanceInterp: if (omega_roll > 0.1 .and. nom_stored > 0 .and. npres > 0) then
  ...
  res_ilo = 0; res_ihi = 0
  do iom=1,nom_stored
   if (omega_stored(iom) <= res_om_hi .and. res_ilo == 0) res_ilo = iom
   if (omega_stored(iom) < res_om_lo) then
    res_ihi = iom - 1
    exit
   endif
   if (iom == nom_stored) res_ihi = nom_stored
  enddo
  if (res_ilo > 0 .and. res_ihi >= res_ilo) then
   res_ileft = res_ilo - 1                           !higher-freq side (lower index)
   res_iright = res_ihi + 1                           !lower-freq side (higher index)
   write(6,'(a,f7.4,a,f7.4,a,f7.4,a)') &
    ' Resonance interpolation: omega_roll=', omega_roll, &
    '  window=[', res_om_lo, ',', res_om_hi, ']'
   write(6,'(a,i3,a,i3,a,i3,a,i3)') &
    ' Indices: lo=', res_ilo, ' hi=', res_ihi, &
    ' left=', res_ileft, ' right=', res_iright
   if (res_ileft >= 1 .and. res_iright <= nom_stored) then
    do iv=1,nv
     do imu=1,nmu
      res_feta_left = feta_stored(iv,imu,res_ileft)
      res_feta_right = feta_stored(iv,imu,res_iright)
      do iom=res_ilo,res_ihi
       !Linear interpolation in omega between boundary points
       res_wt = (omega_stored(iom) - omega_stored(res_iright)) &
              / (omega_stored(res_ileft) - omega_stored(res_iright))
       feta_stored(iv,imu,iom) = res_feta_right &
              + res_wt * (res_feta_left - res_feta_right)
      enddo
     enddo
    enddo
     write(6,'(a,i3,a)') ' Interpolated feta at ', res_ihi-res_ilo+1, ' frequency points'
     !Write interpolated feta to debug file for all speeds/headings/frequencies in window
     do iom=res_ilo,res_ihi
      do iv=1,nv
       do imu=1,nmu
        write(34,'(a,f8.4,a,f8.1,a,i2,a,g14.6)') &
         ' DRIFT_INTERP omega=',omega_stored(iom),' mu=',mu(imu)*180./pi, &
         ' iv=',iv,' feta_interp=',feta_stored(iv,imu,iom)
       enddo
      enddo
     enddo
   else
    write(6,'(a)') ' Resonance at edge of frequency range — skipping interpolation'
   endif
  else
