...
C
      ...
      YPLT(YY) = (YY - YOFP)*GSF
C
C---- don't try to plot if graphics device is not open
      IF(IDEV.EQ.0) RETURN
C
...
C-----------------------------------------
      ...
      INCLUDE 'XCAV.INC'
C
C---- skip if no graphics device is active
      IF(IDEV.EQ.0) RETURN
C
...
C
     ...
     &               SH,ISYM,0.0,0)
      ...
      ENDIF
C
C---- add cavity overlay if cavitation active
      IF(LCAV) CALL CPCAV
C
...
C
C------------------------------------------------------------------
      LOGICAL LVISC, LFOREF, LVCONV, LCAV
      CHARACTER*(*) NAME
      REAL MINF, SIGMA
      REAL ACRIT(*)
C
...
C
      ENDIF
C
      IF(LCAV .AND. SIGMA.GT.0.0) THEN
       YL = YL - YSPACE
       CALL PLMATH(XL        ,YL,1.2*CCH,'s'   ,0.0,1)
       CALL PLCHAR(XL        ,YL,CCH,    '   = ',0.0,5)
       CALL PLNUMB(XL+5.0*CCH,YL,CCH, SIGMA    ,0.0,3)
      ENDIF
C
