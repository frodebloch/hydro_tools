...
C------ find max pivot index NX
  ...
  111      NX = N
   11   CONTINUE
C
        IF(ABS(Z(NX,NP)).LT.1.0E-30) THEN
         PIVOT = 1.0/SIGN(1.0E-30, Z(NX,NP)+1.0E-60)
        ELSE
         PIVOT = 1.0/Z(NX,NP)
        ENDIF
C
...
C------ normalize first row
        IF(ABS(VA(1,1,IV)) .LT. 1.0E-30) VA(1,1,IV) = 1.0E-30
        PIVOT = 1.0 / VA(1,1,IV)
...
C------ normalize second row
        IF(ABS(VA(2,2,IV)) .LT. 1.0E-30) VA(2,2,IV) = 1.0E-30
        PIVOT = 1.0 / VA(2,2,IV)
        DO 20 L=IV, NSYS
...
C------ normalize third row
        IF(ABS(VM(3,IV,IV)) .LT. 1.0E-30) VM(3,IV,IV) = 1.0E-30
