...
CC      EXN_AC = -20.0    *EXN
       ...
       ENDIF
C
       DAX    = EXN    * 0.002/MAX(T1+T2, 1.0E-10)
CC     DAX_AC = EXN_AC * 0.002/MAX(T1+T2, 1.0E-10)
       DAX_A1 = EXN_A1 * 0.002/MAX(T1+T2, 1.0E-10)
       DAX_A2 = EXN_A2 * 0.002/MAX(T1+T2, 1.0E-10)
       DAX_T1 = -DAX/MAX(T1+T2, 1.0E-10)
       DAX_T2 = -DAX/MAX(T1+T2, 1.0E-10)
C
c
c        DAX    = 0.
c        DAX_A1 = 0.
...
C
C---- bail out if any primary variables are NaN
      IF(T1.NE.T1 .OR. T2.NE.T2 .OR.
     &   D1.NE.D1 .OR. D2.NE.D2 .OR.
     &   U1.NE.U1 .OR. U2.NE.U2) THEN
        AMPL2 = AMPL1
        GO TO 101
      ENDIF
C
...
C----------------------------------------------------------
      ...
      INCLUDE 'XBL.INC'
C
C---- guard primary variables against NaN/zero (prevents SIGFPE)
      IF(U2.NE.U2) U2 = 1.0E-10
      IF(T2.NE.T2) T2 = 1.0E-10
      IF(D2.NE.D2) D2 = 1.0E-10
C
...
C
C---- guard against zero/negative/NaN theta and delta*
      IF(T2.LE.0.0 .OR. T2.NE.T2) T2 = 1.0E-10
      IF(D2.LE.0.0 .OR. D2.NE.D2) D2 = 1.0E-10
C
...
C
C---- guard HERAT against non-positive values
      IF(HERAT.LE.1.0E-6) HERAT = 1.0E-6
C
...
C---- set momentum thickness Reynolds number
      IF(V2.LE.0.0 .OR. V2.NE.V2) V2 = 1.0E-20
      IF(U2.LE.0.0 .OR. U2.NE.U2) U2 = 1.0E-10
      IF(R2.LE.0.0 .OR. R2.NE.R2) R2 = 1.0E-10
      RT2    = R2*U2*T2/V2
...
C      Safe guards: protect against NaN, Inf, zero, and negative values
       IF(X2.GT.0.0 .AND. X1.GT.0.0
     &    .AND. X2.LT.1E+10 .AND. X1.LT.1E+10) THEN
        XLOG = LOG(X2/X1)
       ELSE
        XLOG = 1.0
       ENDIF
       IF(U2.GT.1E-20 .AND. U1.GT.1E-20
     &    .AND. U2.LT.1E+10 .AND. U1.LT.1E+10) THEN
        ULOG = LOG(U2/U1)
       ELSE
        ULOG = 0.0
       ENDIF
       IF(T2.GT.1E-20 .AND. T1.GT.1E-20
     &    .AND. T2.LT.1E+10 .AND. T1.LT.1E+10) THEN
        TLOG = LOG(T2/T1)
       ELSE
        TLOG = 0.0
       ENDIF
       IF(HS2.GT.1E-20 .AND. HS1.GT.1E-20
     &    .AND. HS2.LT.1E+10 .AND. HS1.LT.1E+10) THEN
        HLOG = LOG(HS2/HS1)
       ELSE
        HLOG = 0.0
       ENDIF
...
C-    (mainly kicks in at transition)
      IF(ABS(HK1-1.0) .LT. 1.0E-5) THEN
        HK1A = 1.0 + SIGN(1.0E-5, HK1-1.0)
      ELSE
        HK1A = HK1
      ENDIF
      IF(ABS(HK2-1.0) .LT. 1.0E-5) THEN
        HK2A = 1.0 + SIGN(1.0E-5, HK2-1.0)
      ELSE
        HK2A = HK2
      ENDIF
      ARG = ABS((HK2A-1.0)/(HK1A-1.0))
      IF(ARG .LT. 1.0E-10) ARG = 1.0E-10
      HL = LOG(ARG)
      HL_HK1 = -1.0/(HK1A-1.0)
      HL_HK2 =  1.0/(HK2A-1.0)
C
...
C
C
       IF(S2.GT.1E-20 .AND. S1.GT.1E-20
     &    .AND. S2.LT.1E+10 .AND. S1.LT.1E+10) THEN
        SLOG = LOG(S2/S1)
       ELSE
        SLOG = 0.0
       ENDIF
...
ccc   DATA DGR / 0.04 /
      DATA DGR / 0.08 /
C
C---- guard against pathological inputs
      IF(HK.LE.1.0005 .OR. HK.NE.HK) THEN
        AX    = 0.
        AX_HK = 0.
        AX_TH = 0.
        AX_RT = 0.
