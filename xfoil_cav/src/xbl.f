...
C---- march BL with current Ue and Ds to establish transition
      CALL MRCHDU
C
C---- MRCHDU may have modified UEDG at cavity stations (it runs BLSYS
C     which adjusts Ue via the Ue(Hk) relation).  Re-override UEDG
C     at cavity stations back to QCAV before the USAV/UESET swap.
C     Also recompute MASS = DSTR * QCAV for consistency.
C     Additionally, set blended UEDG at smoothing zone stations
C     (ICAV2+1 through ICAV2+NSMOOTH) so the transition is gradual.
      IF(LCAV) THEN
        DO IS=1, 2
          IF(NCAVS(IS) .GT. 0) THEN
            DO IBL = ICAV1(IS), ICAV2(IS)
              IF(LCAVP(IBL,IS)) THEN
                UEDG(IBL,IS) = QCAV
                MASS(IBL,IS) = DSTR(IBL,IS) * QCAV
              ENDIF
            ENDDO
C---------- smoothing zone: blend UEDG from QCAV to inviscid value
C           (UEDG already holds the inviscid value from MRCHDU)
            NSMOOTH = 4
            DO IBL = ICAV2(IS)+1,
...
C
    ...
    7 CONTINUE
C
C---- At cavity stations, UEDG was prescribed = QCAV by the outer
C     loop before SETBL was called.  UESET recomputed a "new" edge
C     velocity from UINV+DIJ*MASS that differs from QCAV.  After
C     the swap above, UEDG = QCAV (old) and USAV = UESET result.
C     Force USAV = UEDG at cavity stations so that DUE2 = UEDG - USAV
C     is naturally zero.  This tells the Newton solver there is no
C     "forced" velocity change at these stations — the prescribed
C     QCAV is the intended edge velocity, not a perturbation.
C     Also force USAV=UEDG at smoothing zone stations.
      IF(LCAV) THEN
        DO IS=1, 2
          IF(NCAVS(IS) .GT. 0) THEN
            DO IBL = ICAV1(IS), ICAV2(IS)
              ...
              ENDIF
            ENDDO
C---------- smoothing zone: also zero DUE2 at blended stations
            NSMOOTH = 4
            DO IBL = ICAV2(IS)+1,
     &               MIN(ICAV2(IS)+NSMOOTH, IBLTE(IS))
              USAV(IBL,IS) = UEDG(IBL,IS)
...
C
C---- force turbulent at cavity stations (free shear layer, no wall)
      IF(LCAV .AND. .NOT.WAKE) THEN
        IF(NCAVS(IS).GT.0 .AND. LCAVP(IBL,IS)) THEN
          TURB = .TRUE.
          TRAN = .FALSE.
        ENDIF
      ENDIF
...
C
C---- At laminar stations, CTI may be uninitialized (= Inf with
C     -finit-real=inf) because only AMI is set from CTAU.
C     When cavitation forces later stations turbulent, S1 from a
C     preceding laminar station carries Inf into BLDIF(2) SLOG.
C     Set CTI to a safe default at all laminar stations.
      IF(IBL.LT.ITRAN(IS)) CTI = 0.03
C
C---- At cavity stations forced turbulent, ensure CTI has a valid
C     turbulent Ctau value even if the station was laminar in Phase 1.
C     CTAU stores amplification factor (AMI) for laminar stations,
C     which is not a valid turbulent shear stress coefficient.
      IF(LCAV .AND. .NOT.WAKE .AND. TURB) THEN
        IF(NCAVS(IS).GT.0 .AND. LCAVP(IBL,IS)) THEN
          IF(IBL.LT.ITRAN(IS)) THEN
            CTI = MAX(CTAU(IBL,IS), 0.03)
            IF(CTI .GT. 0.30) CTI = 0.03
          ENDIF
        ENDIF
      ENDIF
      UEI = UEDG(IBL,IS)
...
C     At cavity stations, USAV was set = UEDG (= QCAV) after the swap
C     above, so DUE2 is naturally zero there — no special case needed.
...
C
      ...
      CALL BLKIN
C
C---- ensure turbulent continuation at post-cavity smoothing zone
C     The cavity was treated as a free shear layer (turbulent, Cf=0).
C     Stations ICAV2+1 through ICAV2+NSMOOTH are in the smoothing zone
C     where UEDG is blended from QCAV to the inviscid value.
C     Keep these stations turbulent for continuity with the cavity.
      IF(LCAV .AND. .NOT.WAKE) THEN
        IF(ICAV2(IS).GT.0 .AND. IBL.GT.ICAV2(IS)
     &     .AND. IBL.LE.MIN(ICAV2(IS)+4, IBLTE(IS))) THEN
         TURB = .TRUE.
         TRAN = .FALSE.
        ENDIF
      ENDIF
...
C---- check for transition and set TRAN, XT, etc. if found
      IF(TRAN) THEN
        ...
        AMI = AMPL2
      ENDIF
      IF(IBL.EQ.ITRAN(IS) .AND. .NOT.TRAN .AND. .NOT.LCAV) THEN
...
C
      ELSE
C
       IF(LCAV .AND. LCAVP(IBL,IS)) THEN
C------- cavitated station: use cavity equation system
         CALL CAVSYS(IBL,IS)
       ELSE
C------- normal (non-cavitated) station
         CALL BLSYS
       ENDIF
C
      ENDIF
...
C
C------ Cavity station bypass: skip MRCHDU Newton iterations at
C       cavitated stations.  MRCHDU uses BLSYS (not CAVSYS) and adjusts
C       Ue via the Ue(Hk) relation, which fights the prescribed UEDG=QCAV.
C       Instead, keep UEDG=QCAV and existing theta/delta*, compute secondary
C       variables, and jump to the store/COM1=COM2 section.
C
C       Post-cavity smoothing zone: NSMOOTH stations after ICAV2 also
C       bypass Newton but use a blended UEDG transitioning from QCAV
C       back to the standard inviscid Ue.  This prevents the sudden
C       discontinuity that causes MRCHDU Newton failure and RMSBL blowup.
C       These stations use standard Cf (NOT Cf=0) — they have wall contact.
        IF(LCAV .AND. .NOT.WAKE) THEN
C
C-------- cavity interior bypass
         IF(NCAVS(IS).GT.0 .AND. IBL.GE.ICAV1(IS)
     &      .AND. IBL.LE.ICAV2(IS) .AND. LCAVP(IBL,IS)) THEN
C-------- force prescribed cavity edge velocity
          UEI = QCAV
C-------- force turbulent state
          TURB = .TRUE.
          TRAN = .FALSE.
          IF(IBL.GE.ITRAN(IS)) THEN
            CTI = CTAU(IBL,IS)
            IF(CTI.LE.0.0 .OR. CTI.GT.0.30) CTI = 0.03
          ELSE
C---------- station was laminar in Phase 1 — seed Ctau
            CTI = 0.03
            ITRAN(IS) = MIN(ITRAN(IS), IBL)
          ENDIF
C-------- establish "2" station BL variables for storage and COM1=COM2
          CALL BLPRV(XSI,AMI,CTI,THI,DSI,DSWAKI,UEI)
          CALL BLKIN
          CALL BLVAR(2)
C-------- guard against pathological secondary variables
C         Same guards as in CAVSYS — prevent NaN from thin cavity BL
          IF(HS2 .LT. 0.15) THEN
            HS2 = 0.15
            HS2_U2 = 0.0
            HS2_T2 = 0.0
          ...
          ENDIF
          CALL BLMID(2)
C-------- cap dissipation (same as CAVSYS)
          IF(DI2 .GT. 0.20) THEN
            DI2    = 0.20
            DI2_S2 = 0.0
            DI2_U2 = 0.0
            ...
            DI2_RE = 0.0
          ENDIF
C-------- zero skin friction (free shear layer — no wall)
          CF2 = 0.0
          CFM = 0.0
C-------- jump to storage section (label 110)
          GO TO 110
         ENDIF
C
C-------- post-cavity smoothing zone bypass
C         NSMOOTH stations after ICAV2, blending UEDG from QCAV to inviscid
         NSMOOTH = 4
         IF(NCAVS(IS).GT.0 .AND. ICAV2(IS).GT.0
     &      .AND. IBL.GT.ICAV2(IS)
     &      .AND. IBL.LE.MIN(ICAV2(IS)+NSMOOTH, IBLTE(IS))) THEN
C-------- blending fraction: 0 at ICAV2+1, approaching 1 at ICAV2+NSMOOTH
          FBLEND = REAL(IBL - ICAV2(IS)) / REAL(NSMOOTH + 1)
C-------- blend from QCAV to current inviscid Ue
          UEINV = UEI
          UEI = (1.0 - FBLEND)*QCAV + FBLEND*UEINV
C-------- force turbulent (continuing from cavity free shear layer)
          TURB = .TRUE.
          TRAN = .FALSE.
          IF(IBL.GE.ITRAN(IS)) THEN
            CTI = CTAU(IBL,IS)
          ...
          ELSE
            ...
            ITRAN(IS) = MIN(ITRAN(IS), IBL)
          ENDIF
C-------- compute BL variables (standard turbulent, WITH wall friction)
          CALL BLPRV(XSI,AMI,CTI,THI,DSI,DSWAKI,UEI)
          CALL BLKIN
          CALL BLVAR(2)
C-------- guard against pathological secondary variables
C         Same guards as cavity interior — smoothing stations may also
C         have thin BL from adjacent cavity free shear layer.
          IF(HS2 .LT. 0.15) THEN
            HS2 = 0.15
            HS2_U2 = 0.0
            HS2_T2 = 0.0
...
C
        SENS = SENNEW
C
C------ NaN/Inf guard on primary variables (MRCHDU)
C       If any primary variable is bad, use upstream station value.
C       This prevents NaN propagation from failed Newton convergence.
        IF(THI.NE.THI .OR. ABS(THI).GT.1.0) THEN
          THI = THET(IBM,IS)
        ENDIF
        IF(DSI.NE.DSI .OR. ABS(DSI).GT.1.0) THEN
          DSI = DSTR(IBM,IS)
        ENDIF
        IF(IBL.GE.ITRAN(IS)) THEN
          IF(CTI.NE.CTI .OR. ABS(CTI).GT.10.0) THEN
            CTI = MAX(CTAU(IBM,IS), 1.0E-6)
          ENDIF
        ENDIF
        IF(UEI.NE.UEI .OR. ABS(UEI).GT.100.0) THEN
          UEI = UEDG(IBM,IS)
        ENDIF
C
C------ store primary variables
        ...
        MASS(IBL,IS) = DSI*UEI
C
C------ NaN guard on secondary stored variables (MRCHDU)
        TAUI = 0.5*R2*U2*U2*CF2
        DISI = R2*U2*U2*U2*DI2*HS2*0.5
        IF(TAUI.NE.TAUI .OR. ABS(TAUI).GT.1E+6) TAUI=TAU(IBM,IS)
        IF(DISI.NE.DISI .OR. ABS(DISI).GT.1E+6) DISI=DIS(IBM,IS)
        TAU(IBL,IS)  = TAUI
        DIS(IBL,IS)  = DISI
        IF(CQ2.NE.CQ2)  CQ2 = CTQ(IBM,IS)
        IF(DE2.NE.DE2)  DE2 = DELT(IBM,IS)
        CTQ(IBL,IS)  = CQ2
        DELT(IBL,IS) = DE2
        IF(HS2.NE.HS2 .OR. T2.NE.T2) THEN
          TSTR(IBL,IS) = TSTR(IBM,IS)
        ELSE
          TSTR(IBL,IS) = HS2*T2
        ENDIF
C
...
C
C-------- at cavity stations Ue is prescribed = QCAV: force no Ue change
          IF(LCAV .AND. IBL.LE.IBLTE(IS)) THEN
            IF(NCAVS(IS).GT.0 .AND. LCAVP(IBL,IS)) THEN
              DUEDG = 0.0
            ENDIF
          ENDIF
C
          IF(ABS(UEDG(IBL,IS)).GT.1.0E-10) THEN
...
C-------- normalize changes
          ...
          IF(IBL.GE.ITRAN(IS)) THEN
            IF(ABS(CTAU(IBL,IS)).GT.1.0E-10) THEN
              DN1 = DCTAU / CTAU(IBL,IS)
            ELSE
              DN1 = 0.0
            ENDIF
          ENDIF
          IF(ABS(THET(IBL,IS)).GT.1.0E-10) THEN
            DN2 = DTHET / THET(IBL,IS)
          ELSE
            DN2 = 0.0
          ENDIF
          IF(ABS(DSTR(IBL,IS)).GT.1.0E-10) THEN
