...
At first glance \SI{225}{\gram/\kilo\watt\hour} may seem surprisingly high:
...
removing some of the Optimiser's pitch/RPM freedom.  The extra fuel therefore
has two components: (i) the raw energy for \SI{150}{\kilo\watt}, and (ii) the
efficiency loss from operating at a constrained, less optimal RPM.  Together
they yield the \SI{225}{\gram/\kilo\watt\hour} effective rate.

With the corrected hull data the engine sits at a higher load fraction even at
\SI{10}{\knot}, so the RPM floor penalty is smaller than previously estimated
--- hence \SI{225}{\gram/\kilo\watt\hour} rather than the earlier
\SI{257}{\gram/\kilo\watt\hour}.

At a typical genset SFOC of \SI{215}{\gram/\kilo\watt\hour}, a separate
auxiliary genset would save approximately \SI{11}{t/yr}
...
#+CAPTION: Daily fuel savings: standard mode (top) vs shaft generator mode (middle), with weather conditions (bottom).  The propeller optimisation saving (blue) is virtually identical in both modes; the wind-assist saving (orange) likewise persists.  Dashed lines show annual means.
#+ATTR_LATEX: :width \textwidth :placement [H]
